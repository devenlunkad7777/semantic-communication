...
const Receiver: React.FC = () => {
  const { inputText, reconstructedText, semanticSimilarity } = useSelector((state: RootState) => state.semantic);
  ...
  useEffect(() => {
    const fetchSimilarity = async () => {
      if (!inputText || !reconstructedText) {
    ...
    fetchSimilarity();
  }, [inputText, reconstructedText, semanticSimilarity]);
  ...
  const getCleanReconstructedText = () => {
    ...
    if (
      reconstructedText.toLowerCase().includes('this is impossible to answer') ||
      reconstructedText.toLowerCase().includes('without knowing the context') ||
      reconstructedText.toLowerCase().includes('impossible to reconstruct')
    ) {
    ...
    const boldTextMatch = reconstructedText.match(/\*\*"(.*?)"\*\*/);
    if (boldTextMatch && boldTextMatch[1]) {
      return boldTextMatch[1];
    }
    
    // Pattern: "The original message was likely: 'text'"
    const likelyMatch = reconstructedText.match(/the original (?:message|sentence) was likely:?\s*["']?(.*?)["']?(?:\.|$)/i) ||
                        reconstructedText.match(/original text:?\s*["']?(.*?)["']?(?:\.|$)/i);
    ...
    const hereWhyIndex = reconstructedText.search(/Here(?:'s| is) why:/i);
    if (hereWhyIndex > 0) {
      return reconstructedText.substring(0, hereWhyIndex).trim();
    }
    
    // Pattern: Anything that starts with "The core meaning of" - usually contains quotes
    const coreMeaningMatch = reconstructedText.match(/The core meaning of ['"]([^'"]*)['"]/i);
    ...
    const quotedTextMatch = reconstructedText.match(/["'](.*?)["']/);
    ...
    const firstSentenceMatch = reconstructedText.match(/^([^.!?]+[.!?])/);
  ...
  const highlightDifferences = () => {
    ...
    return (
      <div className="flex flex-col space-y-4">
        ...
        <div>
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Reconstructed Text:</h3>
